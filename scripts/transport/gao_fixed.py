...
def get_conductivity_gao(mu):
    ...
    for xi, delta_K, E0_K in zip(config.VALLEY_IDX, config.DELTAS, config.E0_ARRAY):
        ...
        for band_idx in range(2):
            sigma_up.append(get_nlt_gao(system_up, band_idx, p, config.T_eff, mu))
            sigma_dn.append(get_nlt_gao(system_dn, band_idx, p, config.T_eff, mu))
...
def main():
    print(12*'=' + " CALCULATING THE CONDUCTIVITY FROM GAO'S FORMULA " + 12*'=')
    print(f"DELTAS: \n{config.DELTAS}")
    print(f"E0_ARRAY: \n{config.E0_ARRAY}")


    sigma_tot_sym, sigma_tot_asym = get_conductivity_gao(config.mu_eff)

    print("\n" + 20*'=' + " FINAL RESULTS GAO " + 20*'=')
    print(f"Longitudinal Non-Linear Conductivity Tensor (Gao):")
    for i in range(2):
        for j in range(2):
            for l in range(2):
                print(f"sigma_{i}{j}{l}: {sigma_tot_sym[i, j, l]:.5}")

    print(f"\nTransverse Non-Linear Conductivity Tensor (Gao):")
    for i in range(2):
        for j in range(2):
            for l in range(2):

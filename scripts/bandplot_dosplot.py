...
from plot_dos import calculate_total_dos
...
def main():
    ...
    if data_path.exists():
        ...
        e_lim = 0.16
        e_vals = np.linspace(-e_lim, e_lim, 400)
    ...
    for i, (xi, (energies_up, energies_dn)) in enumerate(zip(config.VALLEY_IDX, all_energies)):
        # Plot the energies
        axs[i].plot(kx_1d, np.array(energies_up).T, color='red')
        axs[i].plot(kx_1d, np.array(energies_dn).T, color='blue', linestyle='--')
        ...
        axs[i].set_ylim(-0.16, 0.16)
        axs[i].set_xlim(-config.K_LIM, config.K_LIM)
    ...
    axs[0].set_ylabel('Energy (eV)', fontsize=17)

    # Plot DOS
    axs[2].plot(dos_vals, e_vals, color='navy', lw=2)
